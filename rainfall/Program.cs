...
namespace rainfall
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] towns = new string[] {"Rome", "London", "Paris", "NY", "Vancouver", "Sydney", "Bangkok", "Tokyo", "Beijing", "Lima", "Montevideo", "Caracas", "Madrid", "Berlin"};


            Console.WriteLine("Montevideo: " + Mean("Montevideo", data));

            //variance("London", data), 57.42(833333333374)
            Console.WriteLine("Montevideo: " + Variance("Montevideo", data));
        ...
        {
            var splitTowns = strng.Split(new string[] { "\n" }, StringSplitOptions.None);
            string currentTown = "";

            //Finds the town
            foreach (var monthsInTemp in splitTowns)
            {
                if (monthsInTemp.Contains(town))
                {
                    currentTown = monthsInTemp;
                    break;
                }
            }
            if(currentTown == "")
            ...
            MatchCollection matches = rx.Matches(currentTown);

            double totalTemp = 0;
            foreach (Match match in matches)
            {
                totalTemp += Convert.ToDouble(match.Value);
            }
        ...
        public static double Variance(string town, string strng)
        {
            ...
            double townMean = totalTemp / 12;

            //So first I want to create the variance - the standard deviation is just a square root of the variance, so once we are at variance
            //it should be easy from there

            double variance = 0;
            foreach (Match match in matches)
            {
                variance += Math.Pow((Convert.ToDouble(match.Value) - townMean), 2);
            }
